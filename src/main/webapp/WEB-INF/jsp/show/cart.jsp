			...
			<div class="woocommerce-cart container-fluid no-left-padding no-right-padding">
				...
				<div class="container">
					...
					<div class="col-md-12 col-sm-12 col-xs-12 cart-table">
						<form>
						<c:choose>
							<c:when test="${username.isEmpty()}"></c:when>
								<c:otherwise>
									<table class="table table-bordered table-responsive">
										<thead>
											<tr>
												<th class="product-thumbnail">用户</th>
										...
										<c:forEach var="findcartresult" items="${findcartresults }">
											<tr class="cart_item">
												<input id="findcartresultid" type="text" value="${findcartresult.id }" hidden>
												<input id="singleprice" type="text" value="${findcartresult.singleprice }" hidden>
												<td data-title="Item" class="product-thumbnail">${findcartresult.username }</td>
												<td data-title="Product Name" class="product-name"><a href="#">${findcartresult.goodsname }</a></td>
												<td data-title="Quantity" class="product-quantity">
													<div class="prd-quantity" data-title="Quantity">
														<input id="numjian" value="-" class="qtyminus btn" data-field="quantity1" type="button" onclick="jian(${findcartresult.id },${findcartresult.goodsnum })">
														<input id="goodssnum" value="${findcartresult.goodsnum }" class="qty" type="text">
														<input id="numjia" value="+" class="qtyplus btn" data-field="quantity1" type="button" onclick="jia(${findcartresult.id },${findcartresult.goodsnum })">
													</div>
												</td>
												<td data-title="Unit Price" class="product-unit-price">¥${findcartresult.singleprice}</td>
												<td data-title="Total" class="product-subtotal">¥${findcartresult.sumprice}</td>
												<td data-title="Remove" class="product-remove" ><a class="icon icon-Delete" href="javascript:if(confirm('确实要从购物车中删除这个产品吗?'))location='/cart1/del?id=${findcartresult.id }'"></a></td>
												<script type="text/javascript">
												
															function jia(id,goodsnum){
																var datas ={
																		"id":id,
																		"goodsnum":goodsnum
																};
																 $.ajax({
														                type: 'POST',
														                url: '${pageContext.request.contextPath}/cart1/jia',
															...
															function jian(id,goodsnum){
																var datas ={
																 ...
																 $.ajax({
														                type: 'POST',
														                url: '${pageContext.request.contextPath}/cart1/jian',
														                dataType: 'json', //表示返回值的数据类型
														                contentType: 'application/json;charset=UTF-8', //内容类型
														                traditional: true, //使json格式的字符串不会被转码
														                data: JSON.stringify(datas),
					...
					<div class="col-md-4 col-sm-6 col-xs-6 cart-collaterals">
						<div class="cart_totals">
							...
							<div class="wc-proceed-to-checkout">
								<a href="/order/addcart" class="checkout-button button alt wc-forward">提交订单</a>
							</div>
						</div>
					</div>
				</div><!-- Container /- -->
