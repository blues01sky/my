	...
	<div class="table-responsive">
  		<table class="table">
			<tr style="height:50px;">
			   <td class="success" align="center" style="width:150px;">收货人姓名</td>
			  <td class="active" align="center" style="width:200px;">电话</td>
			  <td class="success" align="center" style="width:400px;">地址</td>
			  <td class="active" align="center">订单号</td>
			  <td class="success" align="center">商品名</td>
			  <td class="active" align="center">商品数量</td>
			  <td class="success" align="center">订单状态</td>
			...
			<c:forEach var="listsuper" items="${listsuperorder }">
				...
				<tr style="height:50px;">
				  ...
				  <td class="success" align="center" style="width:150px;">${listsuper.username }</td>
				  <td class="active" align="center" style="width:200px;">${listsuper.telphone }</td>
				  <td class="success" align="center" style="width:400px;">${listsuper.address }</td>
				  <td class="active" align="center">${listsuper.orderid }</td>
				 
				  <td class="success" align="center">${order2.goodsname }</td>
				  <td class="active" align="center">${order2.goodsnum }</td>
				  <td class="success" align="center">${listsuper.zhuangtai }</td>
				  <td class="active" align="center">
					<c:choose>
						<c:when test="${listsuper.zhuangtai=='已支付' }">
							<a href="javascript:if(confirm('确实要取消这个订单吗?成功取消后退款将原路返回！'))location='/order/quxiao?orderid=${listsuper.orderid }'">取消</a>
						</c:when>
						<c:when test="${listsuper.zhuangtai=='已提交' }">
							<a href="javascript:if(confirm('确实要取消这个订单吗?'))location='/order/quxiao?orderid=${listsuper.orderid }'">取消</a>
						  	<a href="/order/yijiaohouzhifu?orderid=${listsuper.orderid }">支付</a>
						</c:when>
						<c:when test="${listsuper.zhuangtai=='用户已取消' }">
							<a href="/order/zaicixiadan?orderid=${listsuper.orderid }">再次下单</a>
						</c:when>
						<c:when test="${listsuper.zhuangtai=='商家已取消' }">
							<a href="connect">联系商家</a>
						</c:when>
						<c:when test="${listsuper.zhuangtai=='已完成' }">
							<a href="connect">联系商家</a>
						</c:when>
						<c:when test="${listsuper.zhuangtai=='已发货' }">
							<a href="connect">联系商家</a>
						</c:when>
						<c:otherwise>
							<a>${listsuper.zhuangtai }</a>
						</c:otherwise>
					</c:choose>
				  </td>
